...
float3 sampleRadiance(
    ...
    thread float3 &primaryAlbedo, thread float3 &primaryNormal,
    thread float3 &primaryPosition, thread float &primaryDepth,
    thread float &primaryRoughness, thread float &primaryHitDistance,
    ...
    for (uint depth = 0; depth <= bounceLimit; ++depth) {
        ...
        if (!foundSurface) {
            ...
            spectralPath.radiance +=
                spectralPath.throughput * misWeight *
        ...
        float albedo = evaluateReflectance(albedoRgb, spectralPath);
        float emissive = evaluateEmission(emissiveRgb, spectralPath);
        float ior = evaluateIorAtWavelength(baseIor, spectralPath);
        ...
        float direct = evalDirectLightingPBR(
        ...
        if (depth == 0 && sceneData.ambientIntensity > 0.0) {
            ...
            float ambientF0 = mix(dielectricF0, albedo, metallic);
            float ambientF = F_Schlick(max(dot(N, V), 0.0), ambientF0);
            float ambientDiffuse = (1.0 - ambientF) * (1.0 - metallic) *
                                   albedo * (1.0 - transmittance);
            float ambientSpecular = ambientF * mix(1.0, 0.35, roughness) *
                                    (1.0 - transmittance);
            ...
            spectralPath.radiance +=
                ...
                (ambientDiffuse + ambientSpecular) * ambientRadiance *
                aoVisibility;
        }
        ...
        float viewFresnel = F_Schlick(NdotV, F0);
        float fresnelProbability = clamp(viewFresnel, 0.001, 0.999);
        ...
        if (sceneData.environmentEnabled != 0 &&
            ...
            if (NdotEnvironment > 0.0 &&
                ...
                float visibility = traceShadowVisibility(
                    ...
                    vertices, indices, instanceData, sceneData, spectralPath,
                    PT_MATERIAL_TEXTURE_ARGS);
        ...
        if (choice < specProb && specProb > 1e-4) {
            if (roughness <= 0.025 || totalInternalReflection) {
                ...
                float F = totalInternalReflection ? 1.0
                                                  : F_Schlick(NdotV, F0);
            ...
            float F = F_Schlick(fresnelCosine, dielectricF0);
            float tint = mix(1.0, albedo, 0.15);
            bounceWeight =
                (1.0 - F) * tint / max(transmitProb, 1e-4);
        } else {
            ...
            float F = F_Schlick(max(dot(V, H), 0.0), F0);
            float kD = (1.0 - F) * (1.0 - metallic) *
                       (1.0 - transmittance);
            float diffuseFactor = disneyDiffuseFactor(
                NdotV, NdotL, max(dot(nextDirection, H), 0.0), roughness);
            float diffuseBsdf = kD * albedo * diffuseFactor / M_PI_F;
            float conditionalPdf = NdotL / M_PI_F;
            float combinedPdf = diffuseProb * conditionalPdf;
            bounceWeight =
                diffuseBsdf * NdotL / max(combinedPdf, 1e-6);
            sampledBsdfPdf = combinedPdf;
            sampledEnvironmentPdf = conditionalPdf;
            sampledEventWasDelta = false;
        }

        bounceWeight = clamp(bounceWeight, 0.0, 16.0);
        spectralPath.throughput *= bounceWeight;
        spectralPath.throughput = min(spectralPath.throughput, 32.0);
        if (depth == 0) {
            spectralPath.throughput *= max(sceneData.indirectStrength, 0.0);
        }
        if (!isfinite(spectralPath.throughput) ||
            spectralPath.throughput < 1e-5) {
            break;
        }

        if (depth >= 2) {
            float survival =
                clamp(spectralPath.throughput, 0.05, 0.95);
    ...
    float3 xyz = spectralRadianceToXYZ(
        max(spectralPath.radiance, 0.0), spectralPath);
